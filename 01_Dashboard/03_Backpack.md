...
(async () => {
    ...
    try {
        ...
        if (!hasItems) {
            container.createEl('div', { text: "Your inventory backpack is empty.", attr: { style: "color: var(--text-muted); font-size: 0.9em; padding: 10px 0; text-align: center;" }});
            return;
        }
        ...
        for (let id in inventory) {
            ...
            card.createEl('div', {text: "In stock: " + qty + " pcs.", attr: {style: 'color: #3498db; font-weight: bold; margin-bottom: auto; font-size: 0.95em;'}});

            const useBtn = card.createEl('button', { text: 'Use Item', cls: 'inv-btn' });
            useBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    setTimeout(() => { 
                        if (useBtn && document.body.contains(useBtn)) useBtn.innerText = "Consumed"; 
            ...
            sellBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    new Notice("💰 Item sold! Refunded " + itemInfo.cost + " GP.");
                } finally {
                    setTimeout(() => { 
        ...
        container.createEl('p', {text: "Inventory System Error: " + e.message, attr: {style: "color:red;"}});
    }
})();
```